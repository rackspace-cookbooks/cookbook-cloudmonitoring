...
action :create do
  ...
  if new_resource.example_id
    fail Exception, 'Cannot specify example_id and criteria' unless new_resource.criteria.nil?
  ...
  if new_resource.check_label
    fail Exception, 'Cannot specify check_label and check_id' unless new_resource.check_id.nil?
  ...
  if notification_plan_id.nil?
    fail ValueError, "Must specify 'notification_plan_id' in alarm resource or in node['cloud_monitoring']['notification_plan_id']"
  end
  ...
  if @current_resource.nil?
    ...
    # Compare attributes
    if !alarm.compare? @current_resource
...
def load_current_resource
  if @new_resource.entity_label
    fail Exception, 'Cannot specify entity_label and entity_id' unless @new_resource.entity_id.nil?
