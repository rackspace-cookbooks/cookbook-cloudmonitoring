...
def load_current_resource
  ...
  if @current_resource == nil then
    ...
    node.set[:rackspace_cloudmonitoring]['agent']['token'] = @current_resource.identity unless @current_resource.nil?
    node.set[:rackspace_cloudmonitoring]['agent']['id'] = @current_resource.label unless @current_resource.nil?
  end
end
